...
export class VortexMo2Service {
  ...
  root(): string {
    ...
    if (!server) return "";
    return this.options.settings.modpackPath(server.key)
  ...
  async status(): Promise<ModpackStatus> {
    ...
    return {
      ...
      installed: Boolean(server && fs.existsSync(this.profileFile("modlist.txt"))),
      currentVersion: server?.modpack ? String(server.modpack.revision) : undefined,
      availableVersion: server?.modpack ? String(server.modpack.revision) : undefined,
      root,
  ...
  async preflight(): Promise<PreflightReport> {
    ...
    const bundledRuntime = path.join(this.options.runtimeSource, "ModOrganizer.exe");
    const installedRuntime = path.join(this.root(), "ModOrganizer.exe");
    if (!fs.existsSync(bundledRuntime)) {
      checks.push({
        id: "client-runtime",
        status: "error",
        message: "The signed Launcher distribution does not contain the global SkyMP/MO2 runtime. Reinstall or update the Launcher.",
      });
    } else if (!fs.existsSync(installedRuntime)) {
      checks.push({
        id: "client-runtime",
        status: "repairable",
        message: "The global SkyMP/MO2 runtime must be prepared for this server.",
      });
    } else {
      checks.push({
        id: "client-runtime",
    ...
    if (!server) {
      checks.push({ id: "server", status: "error", message: "No game server is selected." });
    } else if (!server.modpack) {
      checks.push({ id: "modpack", status: "ok", message: "This server has no Nexus Collection." });
    } else {
      ...
      if (!receipt
        ...
        checks.push(invalid.length
          ? { id: "modpack-integrity", status: "repairable", message: `Repair with Vortex: ${invalid[0]} is missing or changed.` }
          : { id: "modpack-integrity", status: "ok", message: "Nexus revision, plugins, load order and hashes are valid." });
      }
    }
    return {
      ready: !checks.some((item) => item.status === "error" || item.status === "repairable"),
  ...
  async install(): Promise<void> {
    ...
    if (!receipt
      ...
      throw new Error("Vortex was opened. Finish the Collection installation, then run Repair again.");
    }
    await this.materialize(receipt, server.modpack.plugins, server.modpack.loadOrder);
    const invalid = await this.verifyHashes(server.modpack.hashes);
    if (invalid.length) throw new Error(`Vortex staging does not match the Directory manifest: ${invalid[0]}`);
  }
  ...
  private async readReceipt(serverId: string): Promise<VortexReceipt | null> {
    try {
      const value = JSON.parse(await fs.promises.readFile(this.receiptPath(serverId), "utf8")) as VortexReceipt;
      return value.serverId === serverId && Array.isArray(value.mods) ? value : null;
    } catch {
      return null;
    }
  }

  private async materialize(receipt: VortexReceipt, plugins: string[], loadOrder: string[]): Promise<void> {
    const root = this.root();
    await Promise.all(["mods", "profiles/Frostfall", "downloads", "overwrite"].map((item) =>
    ...
    await writeMo2Profile(root, receipt.mods.map((mod) => mod.name), plugins, loadOrder);
  }

  private async verifyHashes(hashes: Record<string, string>): Promise<string[]> {
    ...
    for (const [relative, expected] of Object.entries(hashes)) {
      ...
      try {
        if ((await sha256File(target)).toLowerCase() !== expected.toLowerCase()) invalid.push(relative);
      } catch {
        invalid.push(relative);
      }
    }
...
export async function materializeGlobalRuntime(
  ...
  if (!fs.existsSync(path.join(source, "ModOrganizer.exe")))
    throw new Error("Global SkyMP/MO2 runtime is missing from the Launcher distribution.");
  await linkTree(source, destination);
}

export function assertManagedRoot(root: string, skyrimPath: string, serverKey: string): string {
  if (!root || !path.isAbsolute(root)) throw new Error("MO2 location must be an absolute local path.");
  if (process.platform === "win32" && (!/^[A-Za-z]:[\\/]/.test(root) || root.startsWith("\\\\")))
  ...
  if (normalized.toLowerCase() === game.toLowerCase()
    || normalized.toLowerCase().startsWith(`${game.toLowerCase()}${path.sep}`))
    throw new Error("The managed MO2 root must not be inside the Steam game folder.");
  if (!new RegExp(`(?:^|[\\\\/])${escapeRegex(serverKey)}$`, "i").test(normalized))
...
export async function writeMo2Profile(
  ...
  await fs.promises.writeFile(path.join(profile, "modlist.txt"), `${mods.map((item) => `+${safeName(item)}`).join("\n")}\n`);
  await fs.promises.writeFile(path.join(profile, "plugins.txt"), `${plugins.map((item) => `*${item}`).join("\n")}\n`);
  await fs.promises.writeFile(path.join(profile, "loadorder.txt"), `${loadOrder.join("\n")}\n`);
}

export function safeName(value: string): string {
  const result = value.replace(/[<>:"/\\|?*\u0000-\u001f]/g, "_").trim();
  if (!result || result === "." || result === "..") throw new Error("Vortex receipt contains an invalid mod name.");
  return result;
}
